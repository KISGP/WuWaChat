...
import { clearLogs, getViewerState, openDirectory, readLogs } from '@renderer/services/logs'
...
/**
 * @description 根据日志条目级别和原始行状态返回列表行样式。
 * @param entry 待判断样式的日志条目。
...
/**
 * @description 将日志条目拆分为级别、元数据和详情三段文本。
 * @param entry 待格式化的日志条目。
 * @returns 用于日志行布局的三段文本。
 */
...
export function LogTab(): ReactElement {
  ...
  const refreshLogs = async (): Promise<void> => {
    try {
      setLoading(true)
      setErrorMessage('')
      const [nextViewerState, nextEntries] = await Promise.all([getViewerState(), readLogs()])
