...
export function CharacterTab(): ReactElement {
  ...
  const { loading: saving, run: save } = useAsyncAction(async () => {
    ...
    const document = await saveCharacterPrompt(local.id, prompt)
  ...
  const retry = async (): Promise<void> => {
    ...
    try {
      setRegistry(await retryCharacterSync(selected.id))
    } catch (cause) {
  ...
  const openUpdate = async (): Promise<void> => {
    if (!local) return
    try {
      setRemotePrompt(await getPendingRemoteCharacterPrompt(local.id))
      setDialogOpen(true)
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : String(cause))
    }
  ...
  const applyUpdate = async (): Promise<void> => {
    ...
    setApplying(true)
    try {
      await applyPendingRemoteCharacterPrompt(local.id)
