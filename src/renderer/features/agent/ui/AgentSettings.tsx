...
import { getUnifiedSettings, saveAgentSettings, testMoeGirlpedia } from '@renderer/services/settings'
...
export function AgentTab(): ReactElement {
  ...
  async function saveSettings(next: AgentSettingsStore): Promise<void> {
    ...
    setError(null)
    try {
      setSettings(await saveAgentSettings(next))
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : String(cause))
    }
  }
  ...
  async function testMoeGirlpediaConnection(): Promise<void> {
    ...
    try {
      ...
      const result = await testMoeGirlpedia({
