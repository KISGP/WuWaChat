...
前往 [release](https://github.com/KISGP/WuWaChat/releases) 页面下载
...
Agent 设置中的“允许查询萌娘百科”默认关闭。启用前，请在同一页面填写 Bot 用户名和 Bot Password；密码会由主进程使用 Electron safeStorage 加密保存。

萌娘百科当前会拒绝未登录的 API 请求，因此必须配置 Bot Password。不要把密码写入仓库、日志、角色 Prompt 或聊天消息。机器人密码只用于萌娘百科 API 登录，不支持编辑、删除或其他写操作。
